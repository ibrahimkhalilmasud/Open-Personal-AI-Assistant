...
class MainCLITests(unittest.TestCase):
    ...
    def test_main_index_path(self, vault_engine_cls: MagicMock, create_system: MagicMock) -> None:
        args = argparse.Namespace(scan=False, watch=False, index=True, auto_index=False, search=None)
        with patch("main.parse_args", return_value=args), patch("builtins.print"):
            ...
            fake_engine.vector_indexer.index_all.return_value = argparse.Namespace(indexed=1, skipped=0, failed=0)
            vault_engine_cls.return_value = fake_engine
            create_system.return_value = MagicMock(settings=MagicMock())
            main_module.main()
